...
每次运行会建立独立的系统临时目录，包含桌面配置、项目文件、模拟会话及事件日志；输出中的 `Artifacts` 指向该目录。测试不会读取或修改用户日常使用的桌面数据。事件日志只写事件、协议方法和状态类型，不写提示词或错误原文。
...
`npm run test:e2e` 还依次运行 `e2e-lifecycle.cjs` 和 `e2e-recovery.cjs`。前者检查未保存文件退出确认、app.quit 取消、任务与编辑同时存在时取消退出、明确放弃和保存后退出。后者触发现有界面异常处理器并真正重新载入页面，检查旧回合与待批准请求释放、历史恢复、新消息和新批准成功；不声称覆盖操作系统级崩溃。二者均支持同一个打包程序路径环境变量。
