...
- 添加 `.doc` / `.docx` 后自动提取文字，点击附件可预览发送内容。无需安装 Word/WPS，不修改原文件；转换失败保留草稿。
...
Word 文档单个限 10 MB，提取后的文字单个限 1 MB，并计入文本附件总计 4 MB 上限。转换在本机后台进行，最长等待 30 秒。读取正文、文本框、页眉页脚、脚注尾注和批注文字；图片、签章和原始排版不包含在内，不进行 OCR。加密、损坏或无法提取文字的文档会提示失败；超过上限也会拒绝发送，不静默截断。预览与发送时分别读取当前文件，若期间自行编辑文档，请重新预览。
